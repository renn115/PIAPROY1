-- ============================================================================
-- MIGRACIÓN COMPLETA: respuestas_comentarios y mejoras_docente
-- ============================================================================
-- INSTRUCCIONES:
...
BEGIN
    FOR r IN (SELECT policyname FROM pg_policies WHERE tablename = 'respuestas_comentarios') LOOP
        EXECUTE 'DROP POLICY IF EXISTS "' || r.policyname || '" ON respuestas_comentarios';
    END LOOP;
END $$;
...
BEGIN
    FOR r IN (SELECT policyname FROM pg_policies WHERE tablename = 'mejoras_docente') LOOP
        EXECUTE 'DROP POLICY IF EXISTS "' || r.policyname || '" ON mejoras_docente';
    END LOOP;
END $$;
